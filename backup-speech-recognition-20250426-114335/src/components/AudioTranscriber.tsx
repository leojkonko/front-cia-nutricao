...
const AudioTranscriber: React.FC<AudioTranscriberProps> = ({
  ...
  // Função para importar um arquivo de áudio e processá-lo
  const importAudioFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Iniciar a transcrição com o arquivo selecionado
    setIsTranscribing(true);
    setTranscribeProgress(0);
    startProgressSimulation();
    ...
    reader.onload = async (e) => {
      try {
        ...
        // Converter o resultado para um ArrayBuffer e depois para um Blob
        const arrayBuffer = e.target.result as ArrayBuffer;
        const audioBlob = new Blob([arrayBuffer], { type: file.type });

        // Substituir os chunks de áudio capturados pelo microfone pelo arquivo importado
        audioChunksRef.current = [audioBlob];

        console.log(
          `Áudio importado: ${(audioBlob.size / 1024).toFixed(2)} KB`
        );

        // Chamar a função de transcrição diretamente
    ...
    reader.onerror = () => {
      console.error("Erro ao ler o arquivo de áudio");
      completeProgress();
      setIsTranscribing(false);
      onNotification({
        type: "error",
        message: "Erro ao ler o arquivo de áudio. Tente novamente.",
      });
    };
